    ...
    def step(self) -> None:
        ...
        for drone in self.drones:
            if drone.turns_left > 0:
                ...
                drone_path = self.path[(drone.id - 1) % len(self.path)]
                if drone.current_zone != self.manager.end_zone:
                    current_index = drone_path.index(drone.current_zone)
                    next_zone = drone_path[current_index + 1]

                    if next_zone.current_drone < next_zone.max_drones:
